...
class Trainer:
    ...
    def train_augment(self, epoch, **kwargs):
        ...
        if self.args.imporve==1: 
            print("改动")
            train_data = []
            for i in self.loader['labeled_trainloader']:
                train_data.append(i)
            random.shuffle(train_data) 
            bar = tqdm(train_data, total=len(self.loader['labeled_trainloader']), desc="Training")
        else:
            print("未改动")
            bar = tqdm(self.loader['labeled_trainloader'], total=len(self.loader['labeled_trainloader']), desc="Training")
