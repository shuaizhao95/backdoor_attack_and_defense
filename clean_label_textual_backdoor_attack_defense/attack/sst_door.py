...
import sys
import time
from sst_t_sne import TSNE_model
...
def read_iolation_data(file_path):
...
def get_all_data(base_path):
    ...
    try:
        isolation_path = os.path.join(base_path, 'isolation3.txt')    
        isolation_data = read_iolation_data(isolation_path)
        for index in sorted(isolation_data,reverse=True):
...
def train(model, optimizer, device, epoch, save_path, train_loader_clean, dev_loader_clean, test_loader_clean):
    ...
    for idx in range(epoch):
        ...
        if dev_clean_acc > best_dev_acc:
            ...
            #torch.save(model, os.path.join(save_path, f"best_sst.ckpt"))
            torch.save(model.state_dict(), os.path.join(save_path, f"best_sst.ckpt"))
    ...
    #TSNE_model(model, device, train_loader_clean)
...
def main():
    ...
    parser.add_argument('--clean_data_path', type=str, default='data/sst-2/door/')
