...
def evaluation(model, device, loader):
    ...
    with torch.no_grad():
        ...
        loss_idx = np.argsort(np.array(loss_record))
        for o in loss_idx[:10]:
            print(sentence[o])            
        return loss_idx 


def train(model, optimizer, device, epoch, save_path, train_loader_clean, dev_loader_clean, test_loader_clean):
    model.load_state_dict(torch.load(os.path.join(save_path, f"best1.ckpt")))
    loss_idx = evaluation(model, device, train_loader_clean)
    TSNE_model(model, device, train_loader_clean,loss_idx)
...
def main():
    ...
    train_loader_clean = DataLoader(clean_train_dataset, shuffle=False, batch_size=batch_size, collate_fn=bert_fn)
